...
from ultralytics.nn.modules import (C1, C2, C3, GSConv, SLConv, LAPM1, LAPM2, ESD, SNIr, SNI, GSConvE,
                                    Infos, C3k2, C2PSA, PSA, SNIl, A2C2f, SCDown,
                                    C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
...
def attempt_load_one_weight(weight, device=None, inplace=True, fuse=False):
    ...
    for m in model.modules():
        ...
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Segment):
            m.inplace = inplace  # torch 1.7.0 compatibility
        # elif t in nn.Upsample and not hasattr(m, 'recompute_scale_factor'):
        elif isinstance(m, (nn.Upsample, type)) and not hasattr(m, 'recompute_scale_factor'):
            m.recompute_scale_factor = None  # torch 1.11.0 compatibility
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {GSConv, ESD, C3k2, C2PSA, PSA, SLConv, A2C2f, SCDown,
            ...
            args = [c1, c2, *args[1:]]
            if m in {C2PSA, PSA, C3k2, A2C2f, BottleneckCSP, C1, C2, C2f, C3,
                     C3TR, C3Ghost, C3x}:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m in [LAPM1, LAPM2]:
            c1, c2 = ch[f], args[0]
            args = [c1, c2]
        elif m in [SNIr, SNI, SNIl]:
            c1, c2 = ch[f], make_divisible(args[0] * gw, 8)
            args = [c1, c2]
        elif m is Infos:
            c1, c2 = sum(ch[x] for x in f), int(args[0] * gw)
            args = [c1, c2]
