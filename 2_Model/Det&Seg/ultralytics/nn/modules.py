...
class ESD(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=2, g=1, d=1, act=True):
        super().__init__()
        self.cv = Conv(c1, c2, k, s, None, g, d, act)  # x_dense
        self.globalds = nn.AvgPool2d(4, 2, 1)  # x_global
        self.localds = nn.MaxPool2d(4, 2, 1)  # x_local
        # self.fuse = nn.Conv2d(2*c2, c2, 1, 1, bias=False)

    def forward(self, x):
        # y = torch.cat((self.globalds(x), self.localds(x)), dim=1) + self.cv(x)
        # shuffle
        # y = y.reshape(y.shape[0], 2, y.shape[1] // 2, y.shape[2], y.shape[3])
        # y = y.permute(0, 2, 1, 3, 4)
        # y = y.reshape(y.shape[0], -1, y.shape[3], y.shape[4])
        # return self.fuse(y)
        return torch.cat((self.globalds(x), self.localds(x)), dim=1) + self.cv(x)
...
class C2fk(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = SLConv(c1, 2 * self.c, 3, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class Proto(nn.Module):
    ...
    def __init__(self, c1, c_=256, c2=32):  # ch_in, number of protos, number of masks
        ...
        # self.upsample = SNIr(c_, c_)  # nn.Upsample(scale_factor=2, mode='nearest')
