...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False):
    """
    Drop-in replacement for torch.nn.functional.scaled_dot_product_attention (PyTorch 2.x)

    query: (batch, num_heads, seq_len_q, head_dim)
    key:   (batch, num_heads, seq_len_k, head_dim)
    value: (batch, num_heads, seq_len_v, head_dim)
    attn_mask: (optional) mask tensor, broadcastable to (batch, num_heads, seq_len_q, seq_len_k)
    dropout_p: dropout probability (default 0.0)
    is_causal: if True, apply causal mask (prevent attending to future positions)

    returns: attention output tensor (batch, num_heads, seq_len_q, head_dim)
    """
    ...
    if is_causal:
        ...
        causal_mask = torch.tril(torch.ones((q_len, k_len), device=query.device, dtype=torch.bool))
